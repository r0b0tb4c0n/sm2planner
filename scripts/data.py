...
def main():
    classes = ["Assault", "Bulwark", "Vanguard", "Tactical", "Heavy", "Sniper", "Techmarine"]
...
def gen_json(class_name):
    ...
    with urllib.request.urlopen(wiki_url) as url:
        ...
        try:
            gen_perks(root, data_dict, class_name, column_lookup)
        except:
            print(f"{class_name}: Error generating perk list")

        try:
            gen_prestige(root, data_dict, class_name)
        except:
            print(f"{class_name}: Error generating prestige list")

        return(data_dict)

def gen_perks(root, data_dict, class_name, column_lookup):
    ...
    for perk_row in perk_rows[1:]:
        ...
        for perk_td in perk_tds:
            ...
            perk_td_count += 1
            perk = {}
            perk["name"] = perk_td.xpath('.//@data-title')[0]
            description = perk_td.xpath('.//@data-text')[0]
            perk_id = re.sub(r' ', '-', perk["name"].lower())
            perk["id"] = re.sub(r'[^a-zA-Z0-9-]', '', perk_id)
            perk["desc"] = re.sub(r'<.*?>', '', description)
...
def gen_prestige(root, data_dict, class_name):
